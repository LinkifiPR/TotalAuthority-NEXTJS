...
interface SnapshotAuditRendererV1Props {
  snapshotData: any;
  auditData: any;
}

export const SnapshotAuditRendererV1: React.FC<SnapshotAuditRendererV1Props> = ({ 
  snapshotData, 
  auditData 
}) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {(!parsedData.overall_score && !parsedData.discovery_phrases && !parsedData.chatgpt_knowledge) && (
        <Card className="animate-fade-in">
          <CardHeader>
            <CardTitle>Complete Audit Analysis (Locked V1)</CardTitle>
          </CardHeader>
          <CardContent>
