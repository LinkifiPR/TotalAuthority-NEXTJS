...
export const FullAuditThankYouAnimation: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center relative overflow-hidden bg-gradient-to-br from-orange-50 via-purple-50 to-blue-50 rounded-2xl">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {[...Array(12)].map((_, i) => (
          ...
          >
            <div className={`w-3 h-3 ${i % 3 === 0 ? 'bg-orange-400' : i % 3 === 1 ? 'bg-purple-400' : 'bg-blue-400'} rounded-full twinkle`}></div>
          </div>
        ))}
      ...
      <div className="relative mb-12 z-10">
        ...
        <div className={`relative transition-all duration-1000 ${stage >= 1 ? 'scale-100 opacity-100' : 'scale-50 opacity-0'} ${stage >= 2 ? 'robot-bounce' : ''}`}>
          ...
          <div className={`w-24 h-28 bg-gradient-to-b from-orange-500 via-purple-600 to-blue-700 rounded-2xl relative mx-auto shadow-2xl ${stage >= 2 ? 'glow' : ''}`}>
            ...
            {/* Enhanced Arms */}
            <div className="absolute top-10 -left-5 w-4 h-10 bg-gradient-to-b from-orange-400 via-purple-500 to-blue-600 rounded-full transform rotate-12 shadow-lg"></div>
            <div className="absolute top-10 -right-5 w-4 h-10 bg-gradient-to-b from-orange-400 via-purple-500 to-blue-600 rounded-full transform -rotate-12 shadow-lg"></div>
      ...
      <div className={`space-y-6 transition-all duration-1000 delay-300 ${stage >= 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} z-10`}>
        ...
        <div className={`transition-all duration-1000 delay-700 ${stage >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} space-y-3`}>
          <p className="text-xl text-gray-800 font-medium">
            Our team is <span className="text-purple-600 font-bold">hard at work</span> on your full audit!
          </p>
          <p className="text-base text-gray-700">
            We're analyzing your LLM visibility across all major AI platforms...
          </p>
        </div>
        ...
        <div className={`transition-all duration-1000 delay-1200 ${stage >= 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="flex items-center justify-center space-x-3 text-orange-600 mb-6 bg-white/60 backdrop-blur-sm rounded-full px-6 py-3 shadow-lg">
            ...
            <span className="text-sm font-semibold bg-gradient-to-r from-orange-600 to-purple-600 bg-clip-text text-transparent">
              Processing your comprehensive audit...
            </span>
          </div>
        </div>

        <div className={`transition-all duration-1000 delay-1800 ${stage >= 4 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} space-y-4`}>
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4 shadow-sm">
            <p className="text-lg text-green-800 font-medium mb-2">
              ⚡ Professional audit analysis in progress
            </p>
            <p className="text-sm text-green-700">
              Your personalized AI Visibility Report will be delivered as a comprehensive PDF
            </p>
          </div>
          
          <div className="text-center">
            <p className="text-sm text-gray-600 bg-white/50 backdrop-blur-sm rounded-full px-4 py-2 inline-block shadow-sm">
              🔒 Your data is secure • 🎯 Results guaranteed • 📊 Comprehensive analysis
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(20)].map((_, i) => (
          ...
          >
            <div className={`w-1 h-1 ${i % 3 === 0 ? 'bg-orange-400' : i % 3 === 1 ? 'bg-purple-400' : 'bg-blue-400'} rounded-full twinkle shadow-lg`}></div>
          </div>
        ))}
      </div>
    </div>
